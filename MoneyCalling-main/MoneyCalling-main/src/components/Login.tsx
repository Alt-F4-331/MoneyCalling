  ...
  const handleLogin = async () => {
      try {
          ...
          if (response.status === 200) {
              ...
              const token = response.data.token; // Verific? dac? r?spunsul de la backend con?ine un c�mp 'token'
              if (token) {
                localStorage.setItem('token', token);  // Salveaz? token-ul �n localStorage
              } else {
                console.error("Token-ul nu a fost g?sit �n r?spunsul serverului.");
              }
              navigate('/homepage');
          }
      } catch (error) {
